...
def get_llm_client(provider: str, model_alias: str) -> Optional[BaseLlmClient]:
    ...
    if not ClientClass:
        console.print(f"[bold red]Error: Invalid provider "
                      f"'{escape(provider)}'.[/bold red]")
        return None
...
def set_nested_value(data: Dict, path: List[str], value: str):
    ...
    try:
        ...
        console.print("[bold yellow]Warning: Could not set value for path "
...
def translate_data(
    ...
    try:
        ...
        console.print(f"[bold red]Error reading input file: {escape(str(e))}"
                      "[/bold red]")
        sys.exit(1)
    ...
    if not items_to_translate:
        console.print("[yellow]No values found for the specified keys. "
    ...
    try:
        ...
        console.print(f"[bold red]Error writing to output file: "
...
def main():
    ...
    parser.add_argument("output_file",
                        type=Path,
                        help="Path to save the translated JSON file.")
    parser.add_argument(
        "-k", "--key",
        required=True,
        action="append",
