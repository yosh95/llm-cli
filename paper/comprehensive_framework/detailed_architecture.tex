\begin{tikzpicture}[
    ...
    \node [component, fill=green!5, right=of path] (isolated_env) {Isolated Environment \\ \textit{(VM/Container via MCP)}};
...
\node [tierbox, fit=(identity) (mamba), label={[title]above:Tier 2: Behavioral Zero-Trust (Behavior)}] (t2box) {};
    ...
    % Final Execution
    \node [rectangle, draw, fill=blue!10, text width=7em, text centered, rounded corners, minimum height=3em, right=2.0cm of mamba, font=\small\bfseries] (exec) {Secure \\ Tool \\ Execution};

    \path [line] (isolated_env.east) -- +(0.4,0) |- (exec.160);
    \path [line] (mamba.east) -- (exec.west);
    \path [line] (attest.east) -- +(0.4,0) |- (exec.200);

    % MCP Server-Side (Zero Trust)
    \node [rectangle, draw, fill=gray!10, text width=8em, text centered, dotted, right=1.2cm of exec, font=\scriptsize] (server) {MCP Server-Side \\ \textbf{Secure Wrapper} \\ \textit{(Independent RBAC/Audit)}};
    ...
    \path [line] (server) |- (resp);
    \path [line] (resp.west) -- +(-0.8,0) |- (request.south);
