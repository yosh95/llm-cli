...
Tier 4 (Dual LLM) & Malicious Intent & 100.0\% (5/5) \\
\hline
\end{tabularx}
...
As shown in Table \ref{tab:total_perf}, the cumulative overhead is remarkably low, remaining well under one second for the entire security stack on the local security primitives alone. On the primary test hardware (AMD Ryzen 5 8540U, WSL2), the high-risk ML-DSA-87 identity generation was measured at $\approx$706\,ms ($n=10$ trials) after optimization. All cryptographic latencies are measured from pure-Python reference implementations ({\tt dilithium-py}, {\tt kyber-py}); native C/Rust bindings would yield substantially lower values. This "Security Speed Bump" represents only a small fraction of the 500\,ms--3000\,ms RTT typical of cloud-based LLM inference, ensuring that the local security stack does not compromise the agent's responsiveness.

\textit{Measurement Note on ABAC Overhead:} In our measurements, the marginal overhead of adding ABAC claims to the identity token was found to be negligible. Specifically, in some trial runs, the mean latency for token generation with ABAC claims was slightly lower than the baseline (e.g., -18.54\,ms in Table~\ref{tab:total_perf}). We attribute this to measurement jitter and Python's garbage collection cycles during benchmarking, given that the standard deviation ($\sigma \approx 50$\,ms) exceeds the observed difference. This confirms that the computational cost of policy-based claim injection is functionally zero compared to the underlying cryptographic signing operation.
...
\hline
AST + Static Analysis & 0.04 & 0.05 \\
Subprocess Overhead & 70.78 & 27.43 \\
Identity Gen (ML-DSA-44) & 684.48 & 68.20 \\
Identity Gen (ML-DSA-87) & 706.16 & 165.30 \\
KEM Encapsulation (L3) & 3.39 & 4.37 \\
\hline
\textbf{Total (Worst-Case)} & \textbf{780.37} & \textbf{197.15} \\
\hline
\end{tabularx}
...
As shown in Table \ref{tab:dual_llm_perf}, all models achieved 100\% accuracy: TC1--TC5 were correctly passed (false positive rate = 0\%), while TC6--TC10 were correctly blocked (detection rate = 100\%). These results are based on author-designed test vectors and should be interpreted as a demonstration of correctness on representative cases, not as an exhaustive empirical benchmark. The OpenAI model demonstrated competitive performance, maintaining latencies suitable for real-time guardrails. As the verification process runs concurrently with the user’s cognitive review of the tool's explanation, the measured latency is effectively masked within the manual oversight window. Consequently, the impact on the overall end-to-end response time is minimized, maintaining a responsive experience even when the most rigorous security settings are applied.

\begin{table}[htbp]
\caption{Dual LLM Verification Performance (n=10 test cases)}
\begin{center}
\begin{tabularx}{\columnwidth}{|l|l|C|C|}
\hline
\textbf{Provider} & \textbf{Model} & \textbf{Accuracy} & \textbf{Latency (ms)} \\
\hline
Google & Gemini 3.1 Flash Lite & 100\% & $\approx$1707 \\
OpenAI & GPT-5.4 Nano & 100\% & $\approx$1609 \\
Anthropic & Claude Haiku 4.5 & 100\% & $\approx$1889 \\
\hline
\end{tabularx}
