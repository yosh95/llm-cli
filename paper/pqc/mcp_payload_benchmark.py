...
def run_protocol_benchmark() -> None:
    ...
    print(
        f"ML-DSA-Signed Payload Size: {mldsa_total_size:>5} bytes "
        f"({(mldsa_total_size / rsa_total_size):.2f}x increase)"
    )
    print(
        f"Hybrid-Signed Payload Size: {hybrid_total_size:>5} bytes "
    ...
    print(
        "If the raw protocol payload is inadvertently fed back into the LLM history "
        "(e.g., poorly configured agent framework):"
    )
    ...
    print(
        "Conclusion: Critical to strip signatures ('.signatures' key) before "
        "appending MCP tool results to the LLM context to prevent rapid context "
        "exhaustion."
    )
