...
\section{PQC Remote Attestation and OOB Trust}
...
\subsection{Avoiding TOFU through Out-of-Band (OOB) Distribution}
A critical vulnerability in many dynamic agent ecosystems is Trust-On-First-Use (TOFU), where the initial key exchange is susceptible to Man-in-the-Middle (MitM) attacks. To address this, our architecture mandates that public keys and attestation manifests are distributed via a secure \textbf{Out-of-Band (OOB)} channel (e.g., Mobile Device Management or Secure Enclaves) prior to agent deployment. By relying on pre-established OOB trust roots, the CLI tool remains completely self-contained—eliminating the need for external attestation servers that could introduce single points of failure or external dependencies, while guaranteeing the integrity of the agent's initial state.
...
Table \ref{tab:pqc_bench} shows the latencies of NIST PQC primitives measured over 10 iterations using pure Python implementations.
