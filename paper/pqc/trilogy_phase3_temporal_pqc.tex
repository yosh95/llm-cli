...
Table \ref{tab:pqc_bench} shows the latencies of NIST PQC primitives compared to classical RSA.

\begin{table}[htbp]
\caption{ML-DSA vs RSA Performance (Latency in ms)}
\begin{center}
\begin{tabularx}{\linewidth}{|l|C|C|C|}
\hline
\textbf{Algorithm} & \textbf{Sign (ms)} & \textbf{Verify (ms)} & \textbf{Sig Size (B)} \\
\hline
RSA-2048 & 0.19 & 0.03 & 256 \\
ML-DSA-65 & 23.30 & 6.58 & 3,309 \\
\hline
\end{tabularx}
\label{tab:pqc_bench}
\end{center}
...
While ML-DSA-65 introduces a ~23ms signing latency and a 12x increase in signature size, this overhead is less than 2\% of a typical LLM reasoning cycle (usually >1,000ms). The trade-off—gaining immunity against future quantum decryption and current spoofing—is highly favorable for high-stakes autonomous systems.
