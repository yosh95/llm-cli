...
\resizebox{0.85\textwidth}{!}{
    \input{trilogy_phase3_architecture.tex}
}
...
\section{Evaluation}
Benchmarks were performed on an \textbf{AMD Ryzen 5 8540U} system running WSL2.

\subsection{Cryptographic Performance (Empirical)}
Table \ref{tab:pqc_bench} shows the latencies of NIST PQC primitives measured over 10 iterations.

\begin{table}[htbp]
\caption{PQC Performance Metrics}
\begin{center}
\begin{tabularx}{\linewidth}{|l|C|C|}
\hline
\textbf{Algorithm} & \textbf{Op: Latency (ms)} & \textbf{Op: Latency (ms)} \\
\hline
ML-DSA-65 & Sign: 43.12 & Verify: 8.16 \\
ML-KEM-768 & Encaps: 3.61 & Decaps: 4.59 \\
\hline
\end{tabularx}
\label{tab:pqc_bench}
\end{center}
...
While ML-DSA-65 introduces a ~43ms signing latency, this overhead is less than 5\% of a typical LLM reasoning cycle (usually >1,000ms). The trade-off—gaining immunity against future quantum decryption and current spoofing—is highly favorable for high-stakes autonomous systems.

\section{Limitations and Future Work}

\subsection{Scope of Audit Log Integrity: Local Write vs.\ Transport}
The \textit{PQC-Audit-Chain} described in this paper guarantees integrity from the moment a log entry is written to disk: each entry is ML-DSA-signed, chained via SHA-256, and optionally encrypted with ML-KEM.
The responsibility for \textbf{transporting and externally anchoring} these records is intentionally delegated to the operator's existing log-shipping infrastructure (e.g., Splunk Universal Forwarder, Fluentd, or a SIEM agent).
This design decision follows the Unix single-responsibility principle: \texttt{llm-cli} acts as a lightweight CLI tool whose scope ends at the local write boundary.
Operators running in regulated environments are expected to route the JSONL audit log to an immutable log store (e.g., AWS CloudWatch Logs with Object Lock, Azure Immutable Blob Storage, or a permissioned blockchain) using their standard tooling.
The \texttt{AuditAnchoring.create\_external\_anchor()} helper exposes a Merkle Root that these pipelines can consume as a tamper-evident digest.
Future work includes publishing a reference integration guide for common SIEM platforms.

\subsection{Sentinel Cold-Start Behavior}
The Mamba Sentinel begins with randomly initialized weights and enters an effective detection state only after sufficient training data has been collected (\texttt{mode="collect"}).
Anomalies detected during the warm-up period are logged but do not trigger intervention.
Providing a pre-trained baseline checkpoint—derived from a corpus of representative benign reasoning traces—is left as future work.

\subsection{External Evaluation on Public Benchmarks}
The containment effectiveness figures reported across the trilogy (Phases 1--3) are derived from an internal adversarial dataset.
Validation against publicly available benchmarks such as AgentDojo~\cite{agentdojo2024} is planned to provide externally reproducible evidence.
