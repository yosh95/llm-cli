\begin{tikzpicture}[node distance=2cm, auto,
    ...
    \node [block, below of=intent, node distance=3cm] (policy) {Policy Engine \\ (RBAC / Scope)};
    
    % Node: Identity Manager (Signer)
    \node [block, below of=policy, node distance=3cm] (identity) {Identity Manager \\ (JWT Signer)};
    
    % Container for llm-cli Security Runtime
    % Note: Requires \usetikzlibrary{fit} in main file
    \node[container={llm-cli Security Runtime}, fit=(intent) (policy) (identity), inner sep=1.5em] (runtime) {};
