...
\section{Evaluation}
We evaluated the Zero Trust framework on an \textbf{AMD Ryzen 5 8540U} system running WSL2.

\subsection{Execution Latency (Empirical)}
Table \ref{tab:zt_perf} summarizes the latencies measured over 50 iterations. The Mamba Sentinel adds approximately 14ms of overhead to the reasoning process per block of text, which is nearly two orders of magnitude faster than a "Dual-LLM" verifier approach.
...
We tested the Mamba Sentinel against 5 categories of adversarial reasoning (including "DAN" jailbreaks and obfuscated code) and 4 benign samples. After minimal training on a small corpus of safe agent interactions, the Sentinel achieved a \textbf{100.0\% detection rate} for adversarial patterns with a \textbf{25.0\% false positive rate} (Table \ref{tab:detection_acc}).

\begin{table}[htbp]
\caption{Sentinel Detection Accuracy}
\begin{center}
\begin{tabular}{|l|c|}
\hline
