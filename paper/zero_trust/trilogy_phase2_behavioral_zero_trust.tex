...
Traditional monitoring using secondary LLMs (e.g., using GPT-4o-mini to verify GPT-4o's output) is slow (1--2 seconds of latency) and resource-intensive. We utilize a \textbf{Mamba (State Space Model)} implementation \cite{mamba2023} optimized with pure NumPy. Unlike Transformers, Mamba's linear scaling and recurrent state make it ideal for monitoring streaming reasoning traces at a byte level with sub-millisecond overhead per token.
