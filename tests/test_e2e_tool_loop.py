...
from llm_cli.modules.models import Message, Role, ContentPart, DataSource
...
def mock_session(tmp_path):
    ...
    with patch("llm_cli.clients.config.config_manager.load_config", return_value={"security": {}}):
        client = MockClient()
...
def test_react_tool_loop(mock_session):
    ...
    def side_effect_turn1(data):
        tool_call = {"id": "call_123", "name": "exec_mock_tool", "args": {"explanation": "Testing", "input_str": "hello"}}
        ...
        return (("I will call the test tool.", "I need to process some data."), {"prompt_tokens": 10, "candidates_tokens": 20})

    # 2. Second turn: LLM provides final answer after seeing tool result
    def side_effect_turn2(data):
        msg = Message(role=Role.MODEL, parts=[ContentPart(text="The tool returned the processed result.")])
        client.conversation.append(msg)
        return (("The tool returned the processed result.", None), {"prompt_tokens": 50, "candidates_tokens": 10})
    ...
    client.conversation.append(Message(role=Role.USER, parts=[ContentPart(text="Please run the test tool.")]))
