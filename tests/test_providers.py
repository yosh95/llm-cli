...
class TestProviderClients:
    ...
    @pytest.mark.parametrize("client_class,config_section,expected_base64", [
        (GeminiClient, 'google', True),
        (ClaudeClient, 'anthropic', True),
        (GrokClient, 'xai', False),
    ])
    def test_client_initialization(self, mock_config, client_class,
                                   config_section, expected_base64):
    ...
    def test_initial_tool_activation(self, mock_config):
        ...
        client = GeminiClient(
            initial_model_alias="default", stdout=True,
            initial_tools=['search']
        )
...
class TestUnifiedClient:
    ...
    def test_unified_client_initialization(self, mock_config):
        ...
        client = UnifiedClient(
            initial_provider='google', initial_model_alias="default",
            stdout=True
        )
    ...
    def test_unified_pdf_delegation_to_gemini(
            self, mock_config, temp_pdf_file):
...
class TestSystemPrompt:
    ...
    @pytest.mark.parametrize("client_class,config_section", [
        (GeminiClient, 'google'),
        (OpenAIClient, 'openai'),
        (ClaudeClient, 'anthropic'),
        (GrokClient, 'xai'),
    ])
    def test_system_prompt_enabled_by_default(self, mock_config,
    ...
    def test_system_prompt_disabled_with_flag(self, mock_config,
                                              client_class, config_section):
