import pytest
from llm_cli.security.command_validator import CommandValidator, CommandValidationError
...
class TestSecurityBenchmarks:
    ...
    def test_direct_injection(self, validator, payload):
        ...
        assert any(msg in err_msg for msg in ["dangerous pattern", "forbidden", "not allowed", "whitelist"])
    ...
    def test_semantic_attacks(self, validator, payload):
        ...
        with pytest.raises(CommandValidationError) as exc:
            validator.validate(payload)
        err_msg = str(exc.value).lower()
        assert any(word in err_msg for word in ["forbidden", "prohibited", "not allowed", "whitelist", "dangerous pattern"])
    ...
    def test_path_traversal(self, validator, payload):
        ...
        with pytest.raises(CommandValidationError) as exc:
            validator.validate(payload)
        err_msg = str(exc.value).lower()
        assert any(word in err_msg for word in ["traversal", "not allowed", "forbidden", "absolute path"])
