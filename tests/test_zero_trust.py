...
def identity_manager(tmp_path):
    ...
    with patch("llm_cli.security.identity.IdentityManager._KEY_DIR", keys_dir), \
         patch("llm_cli.security.identity.IdentityManager._PRIVATE_KEY_PATH", keys_dir / "id_rsa"), \
         patch("llm_cli.security.identity.IdentityManager._PUBLIC_KEY_PATH", keys_dir / "id_rsa.pub"):
...
def test_token_verification(identity_manager):
    ...
    token = identity_manager.generate_token(user_id="alice", roles=["admin"], audience="server1")
...
def policy_engine():
    config = {
        ...
        "subjects": {
            "banned_user": {"denied_tools": ["edit_file"]}
        }
    }
...
def test_policy_scope_enforcement(policy_engine):
    ...
    # Allowed path
    assert policy_engine.evaluate("edit_file", {"path": "/home/project/main.py"}, context) is True
    
    # Disallowed path (Out of scope)
    assert policy_engine.evaluate("edit_file", {"path": "/etc/passwd"}, context) is False
...
def test_policy_subject_ban(policy_engine):
    ...
    assert policy_engine.evaluate("edit_file", {"path": "/home/project/main.py"}, context) is False
...
def test_global_guardrails(policy_engine):
    ...
    assert policy_engine.evaluate("execute_command", {"command": "rm -rf /"}, context) is False
...
def test_integrity_audit_verification(tmp_path):
    ...
    with patch("pathlib.Path.expanduser", return_value=log_file):
